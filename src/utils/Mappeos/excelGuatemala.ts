...
export class ClassExcelGuatemala {

    static async excelGuatemala(dataToInsert: any, items: any, tipoExcel: string, workbook: any, worksheet: any, boldBorderStyle: any, outputDir: any, path: any): Promise<string> {
        let newFilePath: any = '';
        ...
        if (tipoExcel === 'BEAUTY') {
            ...
            const pesoBruto = worksheet.getCell(`C${realRowsTotal + 1}`);
            pesoBruto.value = dataToInsert.clientPeso;
            pesoBruto.border = boldBorderStyle; // Aplicar borde
    
            const totalCajas = worksheet.getCell(`C${realRowsTotal + 2}`);
    ...
    static async excelGuatemalaVSFA(dataToInsert: any, items: any, tipoExcel: string, workbook: any, worksheet: any, boldBorderStyle: any, outputDir: any, path: any): Promise<string> {
        let newFilePath: any = '';
    
        if (tipoExcel === 'BEAUTY') {
            // --- Insertar datos de cabecera ---
            // Aplica estilos de borde aquí si también quieres los campos de cabecera con bordes
            const clientCodeCell = worksheet.getCell('I7');
            ...
            const invoiceDateCell = worksheet.getCell('I3');
            invoiceDateCell.value = dataToInsert.invoiceDate;
            //invoiceDateCell.border = boldBorderStyle; // Aplicar borde
    
            const invoiceNumberCell = worksheet.getCell('I5');
            invoiceNumberCell.value = dataToInsert.invoiceNumber;
            //invoiceNumberCell.border = boldBorderStyle; // Aplicar borde
            ...
            items.forEach((item: any) => {
                ...
                const cellF = worksheet.getCell(`F${currentRowForNewItems}`); cellF.value = item.CLASE ?? ''; cellF.border = boldBorderStyle;
                const cellG = worksheet.getCell(`G${currentRowForNewItems}`); cellG.value = item.US_TARIFF ?? ''; cellG.border = boldBorderStyle;
                const cellH = worksheet.getCell(`H${currentRowForNewItems}`); cellH.value = item.ORIGEN; cellH.border = boldBorderStyle;
                const cellI = worksheet.getCell(`I${currentRowForNewItems}`); cellI.value = item.MARCA; cellI.border = boldBorderStyle;
                const cellJ = worksheet.getCell(`J${currentRowForNewItems}`); cellJ.value = item.CANTIDAD; cellJ.border = boldBorderStyle;
                const cellK = worksheet.getCell(`K${currentRowForNewItems}`); cellK.value = item.PRECIO; cellK.border = boldBorderStyle;
                const cellL = worksheet.getCell(`L${currentRowForNewItems}`); cellL.value = item.TOTAL; cellL.border = boldBorderStyle;
            ...
            worksheet.pageSetup.printArea = `A1:L${lastRowED}`;
            worksheet.views = [{ state: 'normal', showGridLines: true }];
